...
function generateImage(weeks) {
  ...
  if (lastDay) {
      ...
      const lastDayIndex = weeks[lastWeekIndex].contributionDays.findIndex(day => day.date === lastDay.date);

      if (lastDayIndex !== -1) {
          const x = lastWeekIndex * (squareSize + gapSize) + margin;
          const y = lastDayIndex * (squareSize + gapSize) + 20 + margin;

          ctx.fillStyle = lastDay.contributionCount > 0
  ...
  legendColors.forEach((color, i) => {
      ctx.fillStyle = color;
      ctx.fillRect(legendX + 50 + i * (legendSquareSize + legendGap), legendY, legendSquareSize, legendSquareSize);
  });

  ctx.fillStyle = textColor;
  ctx.fillText("Mais", legendX + 50 + legendColors.length * (legendSquareSize + legendGap) + 10, legendY + 12);

  return canvas.toBuffer("image/png");
}
  
  export { getContributions, generateImage }